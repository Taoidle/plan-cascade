    ...
    >
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            <span className="font-medium text-sm text-gray-900 dark:text-white">{toolCall.name}</span>
        ...
        >
          <statusConfig.Icon className={clsx('w-3.5 h-3.5', toolCall.status === 'executing' && 'animate-spin')} />
      ...
      {canRevert && !isExpanded && (
        <div className="px-3 pb-2">
          <RevertButton toolCall={toolCall} showConfirm={showRevertConfirm} setShowConfirm={setShowRevertConfirm} />
        </div>
      )}
  ...
  return (
    <div className="p-3 space-y-3">
      ...
      <div className="space-y-2">
        <TruncatedText content={toolCall.parameters.file_path || 'N/A'} isPath label="File Path" maxLength={80} />
        {(toolCall.parameters.offset !== undefined || toolCall.parameters.limit !== undefined) && (
          <div className="flex gap-4 text-xs text-gray-500">
            {toolCall.parameters.offset !== undefined && <span>Offset: {toolCall.parameters.offset}</span>}
            {toolCall.parameters.limit !== undefined && <span>Limit: {toolCall.parameters.limit}</span>}
          </div>
        )}
      </div>
      ...
      {/* Error */}
      {toolCall.result?.error && <ErrorDisplay error={toolCall.result.error} />}
    </div>
  );
}
...
function WriteToolContent({ toolCall }: ToolCallContentProps) {
  return (
    <div className="p-3 space-y-3">
      ...
      <div className="space-y-1">
        <Label>Content</Label>
        <TruncatedText content={toolCall.parameters.content || 'No content'} maxLength={300} maxExpandedHeight={400} />
      </div>
    ...
    <div className="p-3 space-y-3">
      ...
      <div className="space-y-1">
        ...
        <div className="bg-gray-900 rounded-lg overflow-hidden">
          ...
          <TruncatedText content={toolCall.parameters.command || 'N/A'} isCommand maxLength={150} className="p-0" />
        </div>
      </div>
    ...
    <div className="p-3 space-y-3">
      ...
      <div className="flex gap-4">
        ...
        {toolCall.parameters.path && (
          <TruncatedText content={toolCall.parameters.path} isPath label="Path" maxLength={60} className="flex-1" />
        )}
      </div>

      {/* Result - Enhanced File list */}
      {toolCall.result?.files && toolCall.result.files.length > 0 && (
        <GlobResultViewer files={toolCall.result.files} pattern={toolCall.parameters.pattern} maxHeight={300} />
      )}
...
function GrepToolContent({ toolCall }: ToolCallContentProps) {
  return (
    <div className="p-3 space-y-3">
      ...
      {toolCall.result &&
        !toolCall.result.matches?.length &&
        !toolCall.result.files?.length &&
  ...
  return (
    <div className="p-3 space-y-3">
      ...
      {!!toolCall.parameters.url && (
        <TruncatedText content={toolCall.parameters.url as string} label="URL" maxLength={80} />
      )}
      {!!toolCall.parameters.query && (
        <TruncatedText content={toolCall.parameters.query as string} label="Query" maxLength={100} />
      )}
      ...
      {toolCall.result?.content && (
        <div className="space-y-1">
          <Label>Result</Label>
          <TruncatedText content={toolCall.result.content} maxLength={500} maxExpandedHeight={400} />
        </div>
      )}
  ...
  return (
    <div className="p-3 space-y-3">
      ...
      <div className="space-y-1">
        ...
        <TruncatedText content={JSON.stringify(toolCall.parameters, null, 2)} isJson maxLength={300} />
      </div>
      ...
      {toolCall.result && (
        <div className="space-y-1">
          <Label>Result</Label>
          <TruncatedText content={JSON.stringify(toolCall.result, null, 2)} isJson maxLength={300} />
        </div>
      )}
    </div>
  );
...
function Label({ children }: { children: React.ReactNode }) {
  return (
    <span className="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">{children}</span>
  );
}
