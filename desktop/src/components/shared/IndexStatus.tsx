...
interface IndexStatusEvent {
  project_path: string;
  status: 'idle' | 'queued' | 'indexing' | 'indexed' | 'indexed_no_embedding' | 'error' | 'stale';
  ...
  if (status === 'queued') {
    return (
      <div className={clsx('flex items-center gap-1.5', className)}>
        <svg
          className={clsx('animate-spin text-slate-500 dark:text-slate-400', compact ? 'w-3 h-3' : 'w-4 h-4')}
          fill="none"
          viewBox="0 0 24 24"
        >
        ...
        <span className={clsx('text-slate-600 dark:text-slate-400', compact ? 'text-xs' : 'text-sm')}>
          {t('indexing.queued')}
        </span>
      </div>
    );
  ...
  if (status === 'stale') {
    return (
      <div className={clsx('flex items-center gap-1.5', className)}>
        ...
        <span className={clsx('text-orange-600 dark:text-orange-400', compact ? 'text-xs' : 'text-sm')}>
          {t('indexing.stale')}
        </span>
        <button
          onClick={handleReindex}
