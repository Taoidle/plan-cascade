...
export function GraphWorkflowList() {
  ...
  const { workflows, currentWorkflowId, loading, fetchWorkflows, selectWorkflow, startNewWorkflow, deleteWorkflow } =
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-semibold text-gray-900 dark:text-white">{t('graphWorkflow.list.title')}</h3>
        <button onClick={startNewWorkflow} className="text-xs text-primary-600 dark:text-primary-400 hover:underline">
      ...
      {loading.list ? (
        ...
        <div className="text-xs text-gray-500 dark:text-gray-400">{t('graphWorkflow.list.empty')}</div>
      ) : (
            ...
            >
              <div className="min-w-0 flex-1">
                ...
                <p className="text-xs text-gray-500 dark:text-gray-400">
                  {t('graphWorkflow.list.nodeCount', { nodes: wf.node_count })}{' '}
                  {t('graphWorkflow.list.edgeCount', { edges: wf.edge_count })}
                </p>
              </div>
              <button
                onClick={(e) => {
