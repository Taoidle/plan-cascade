...
export function PluginPanel() {
  ...
  return (
    <div data-testid="plugin-panel" className="h-full min-h-0 flex flex-col">
      {/* Header */}
      <div className="flex items-center justify-between px-3 py-2 border-b border-gray-200 dark:border-gray-700">
        <span className="text-xs font-medium text-gray-700 dark:text-gray-300">{t('pluginPanel.title')}</span>
        <button
      ...
      <div className="px-2 py-2 space-y-1 flex-1 min-h-0 overflow-y-auto">
        ...
        {loading && plugins.length === 0 && (
          <div className="text-center py-4">
            <span className="text-xs text-gray-400 dark:text-gray-500">{t('pluginPanel.loading')}</span>
          </div>
        )}
        ...
        {!loading && plugins.length === 0 && (
          <div className="text-center py-4">
            <span className="text-xs text-gray-400 dark:text-gray-500">{t('pluginPanel.noPlugins')}</span>
          </div>
        )}
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-1">
                <span className="text-xs text-gray-900 dark:text-white truncate">{plugin.name}</span>
              ...
              <div className="flex items-center gap-1.5 text-2xs text-gray-400 dark:text-gray-500">
                ...
                {plugin.command_count > 0 && <span>{plugin.command_count} cmds</span>}
      ...
      <div className="px-3 py-2 flex gap-2 border-t border-gray-200 dark:border-gray-700">
        <button
          onClick={handleManageAll}
          className={clsx(
