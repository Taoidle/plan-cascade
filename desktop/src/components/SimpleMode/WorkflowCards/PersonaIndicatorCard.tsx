...
export function PersonaIndicatorCard({ data }: { data: PersonaIndicatorData }) {
  ...
  const displayName = resolvePersonaDisplayName(t, data.role, data.displayName);
  ...
  return (
    <div className="inline-flex items-center gap-1.5 px-2 py-1 rounded-md bg-gray-100 dark:bg-gray-800 border border-gray-200 dark:border-gray-700">
      ...
      <span className="text-2xs font-medium text-gray-700 dark:text-gray-300">{displayName}</span>
      {data.model && (
      ...
      <span className="text-2xs text-gray-400 dark:text-gray-500">
        {t(`workflow.persona.active`, { name: displayName, defaultValue: '{{name}} is working...' })}
      </span>
    </div>
  );
}
