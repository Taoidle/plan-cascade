...
export function AgentCard({ agent, onRun, onEdit, onDelete, isSelected = false }: AgentCardProps) {
  ...
  return (
    ...
    >
      ...
      {/* Description */}
      {agent.description && (
        <p className="text-sm text-gray-600 dark:text-gray-400 mb-3 line-clamp-2">{agent.description}</p>
      )}

      {/* Stats Row */}
      <div className="flex items-center gap-4 mb-3 text-xs text-gray-500 dark:text-gray-400">
        <span>
          {stats.total_runs} {t('runs', { count: stats.total_runs })}
        </span>
