...
//!
//! `ToolExecutionContext` carries all state that tools need. Service
//! fields that depend on main-crate types (IndexStore, EmbeddingService,
//! WebFetchService, etc.) are stored as `Arc<dyn Any + Send + Sync>`
//! and downcast by tool implementations in the main crate. This keeps
//! the tools crate free of orchestrator dependencies.
...
///
/// Service fields (web_fetch, index_store, embedding_service, etc.) use
/// `Arc<dyn Any + Send + Sync>` for decoupling. Tool implementations in
/// the main crate downcast these to their concrete types.
pub struct ToolExecutionContext {
    ...
    /// Downcast to `WebSearchService` in the main crate.
    pub web_search: Option<Arc<dyn Any + Send + Sync>>,
    /// Optional index store for CodebaseSearch tool.
    /// Downcast to `IndexStore` in the main crate.
    pub index_store: Option<Arc<dyn Any + Send + Sync>>,
    /// Optional embedding service for semantic search.
    /// Downcast to `EmbeddingService` in the main crate.
    pub embedding_service: Option<Arc<dyn Any + Send + Sync>>,
    /// Optional EmbeddingManager for provider-aware semantic search (ADR-F002).
    /// Downcast to `EmbeddingManager` in the main crate.
    pub embedding_manager: Option<Arc<dyn Any + Send + Sync>>,
    /// Optional HNSW index for O(log n) approximate nearest neighbor search.
    /// Downcast to `HnswIndex` in the main crate.
    pub hnsw_index: Option<Arc<dyn Any + Send + Sync>>,
    ...
    /// Downcast to `TaskContext` in the main crate.
    /// When None, the Task tool returns a depth-limit error.
    pub task_context: Option<Arc<dyn Any + Send + Sync>>,
}
...
impl ToolExecutionContext {
    ...
    /// Downcast a type-erased service field.
    ///
    /// Convenience method for tool implementations to retrieve strongly-typed
    /// service references from the Any fields.
    ///
    /// # Example
    /// ```ignore
...
impl ToolRegistry {
    ...
    pub fn refresh_toolsets(&mut self, filter_ctx: &ToolFilterContext) {
        ...
        let mut toolset_names: HashSet<String> = HashSet::new();
...
mod tests {
    ...
    fn make_test_context() -> ToolExecutionContext {
        // Use a simple struct that implements Any for the web_fetch field
        struct DummyWebFetch;
        ToolExecutionContext {
            ...
            web_fetch: Arc::new(DummyWebFetch),
            web_search: None,
            index_store: None,
