...
impl ToolExecutor {
    ...
    ///
    /// When `scope="all"`, uses `HybridSearchEngine` for RRF-fused multi-channel
    /// search (FTS5 BM25 + semantic HNSW). Falls back to manual channel logic on error.
    async fn execute_codebase_search(&self, args: &serde_json::Value) -> ToolResult {
        ...
        if scope == "all" {
            match self.execute_hybrid_search(query, &project_path, component, index_store).await {
                Ok(output) => return ToolResult::ok(output),
                Err(e) => {
                    // Fallback: log the error and try individual channels below
                    tracing::warn!("HybridSearchEngine failed, falling back to manual channels: {}", e);
                }
            }
        }
        ...
        if scope == "symbols" || scope == "all" {
            self.search_symbols_channel(query, component, &project_path, scope, index_store, &mut output_sections);
        }

        // --- File search ---
        if scope == "files" || scope == "all" {
            self.search_files_channel(query, component, &project_path, scope, index_store, &mut output_sections);
        }

        // --- Semantic search ---
        if scope == "semantic" || scope == "all" {
            self.search_semantic_channel(query, &project_path, scope, index_store, &mut output_sections).await;
        }
        ...
        if let Some(ref emb_mgr) = self.embedding_manager {
            ...
            if emb_svc.is_ready() {
                ...
                if !query_embedding.is_empty() {
                    let search_result = index_store.semantic_search(&query_embedding, project_path, 10);
                    Self::format_semantic_results(query, search_result, output_sections);
                } else {
                    output_sections.push(
    ...
    fn format_semantic_results(
        query: &str,
        search_result: Result<Vec<crate::services::orchestrator::embedding_service::SemanticSearchResult>, crate::utils::error::AppError>,
        output_sections: &mut Vec<String>,
    ) {
        match search_result {
