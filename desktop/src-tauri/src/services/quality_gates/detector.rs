...
impl ProjectDetector {
    ...
    pub fn detect(&self) -> AppResult<ProjectDetectionResult> {
    ...
    fn extract_nodejs_metadata(&self, marker_path: &Path) -> AppResult<ProjectMetadata> {
        let content = std::fs::read_to_string(marker_path)?;
        let json: serde_json::Value = serde_json::from_str(&content)
            .map_err(|e| AppError::parse(format!("Failed to parse package.json: {}", e)))?;
    ...
    fn extract_rust_metadata(&self, marker_path: &Path) -> AppResult<ProjectMetadata> {
        ...
        // Check for clippy in dependencies (usually always available)
    ...
    fn extract_go_metadata(&self, marker_path: &Path) -> AppResult<ProjectMetadata> {
        let content = std::fs::read_to_string(marker_path)?;
        let mut metadata = ProjectMetadata::default();
        ...
        // Go always has tests through go test
        metadata.has_tests = true;
        metadata.test_framework = Some("go test".to_string());

        // Go has go vet and staticcheck as linting
...
pub fn detect_project_type(project_path: impl AsRef<Path>) -> AppResult<ProjectDetectionResult> {
