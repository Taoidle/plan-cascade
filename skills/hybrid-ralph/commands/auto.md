---
description: "Generate PRD from task description and enter review mode. Auto-generates user stories with priorities, dependencies, and acceptance criteria for parallel execution. Usage: /hybrid:auto [--flow <quick|standard|full>] [--tdd <off|on|auto>] [--confirm] [--no-confirm] [--spec <off|auto|on>] [--first-principles] [--max-questions N] [--agent <name>] <task description> [design-doc-path]"
---
...
Parameters are saved to `prd.json` and later used by `/approve`. Priority order:

1. **Command-line flags to THIS command** (highest priority)
   - Example: `/hybrid:auto --flow full "task"`
   - Saved to `prd.json` as `flow_config`, `tdd_config`, etc.

2. **Command-line flags to `/approve`**
   - Can override values saved in `prd.json`
   - Example: Even if prd.json has `flow_config.level="standard"`, running `/approve --flow full` will use `full`
...
/hybrid:auto --flow full --tdd on "Build auth"
...
/approve --flow standard
# → Uses flow="standard" (overrides prd.json), tdd="on" (from prd.json)
```

**Note:** This command writes parameters to `prd.json` in Step 5. The recommended `/approve` command (shown in Step 6) includes the same parameters for clarity, but they're already in prd.json.
...
echo "  3. Used by /approve when executing stories"
echo "============================================================"
echo ""
```
...
Optional arguments:
  - Flow control: /hybrid:auto --flow full "task"
  - TDD mode: /hybrid:auto --tdd on "task"
  - Confirm mode: /hybrid:auto --confirm "task"
  - No confirm: /hybrid:auto --flow full --no-confirm "task"
  - Spec interview: /hybrid:auto --flow full --spec on "task"
  - First principles: /hybrid:auto --flow full --spec on --first-principles "task"
  - Limit interview: /hybrid:auto --flow full --spec on --max-questions 12 "task"
  - Design document path: /hybrid:auto "task" ./design.md
  - Agent override: /hybrid:auto --agent codex "task"

Example with full flow (CI-friendly, no prompts):
  /hybrid:auto --flow full --tdd on --no-confirm "Implement user authentication"
```
...
- Full flow CI-friendly: `/approve --flow full --tdd on --no-confirm`

## Notes

- If PRD validation fails, show errors and suggest `/edit` to fix manually
