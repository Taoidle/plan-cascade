...
class IterationLoop:
    ...
    def _wait_for_batch_completion(
        ...
        while True:
            ...
            # Poll for completion
            if self.orchestrator:
                batch_status = self.orchestrator.check_batch_complete(stories)
    ...
    def _handle_retry(
        ...
        self.retry_manager.record_failure(
            story_id=story_id,
            agent="unknown",  # Will be updated when we get more info
        ...
        # Re-launch the story (will be picked up in next poll cycle)
        # The orchestrator needs to be told to retry this story
        # This is handled by the state being updated
