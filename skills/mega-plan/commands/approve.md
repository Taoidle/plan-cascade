---
description: "Approve the mega-plan and start feature execution. Creates worktrees and generates PRDs for each feature. Usage: /mega:approve [--flow <quick|standard|full>] [--tdd <off|on|auto>] [--confirm] [--no-confirm] [--spec <off|auto|on>] [--first-principles] [--max-questions N] [--auto-prd] [--agent <name>] [--prd-agent <name>] [--impl-agent <name>]"
---
...
1. **Command-line flags to THIS command** (highest priority)
   - Example: `/mega:approve --flow full --tdd on`
   - Overrides all other sources

2. **mega-plan.json configuration** (saved by `/mega:plan`)
...
1. **Use Read tool for file reading** - NEVER use `cat`, `head`, `tail` via Bash
   - Use `Read("mega-plan.json")`
   - Do NOT use `Bash("cat mega-plan.json")`

2. **Use Glob tool for file finding** - NEVER use `find`, `ls` via Bash
   - Use `Glob(".worktree/*/progress.txt")`
   - Do NOT use `Bash("ls .worktree/*/progress.txt")`

3. **Use Grep tool for content search** - NEVER use `grep` via Bash
   - Use `Grep("[FEATURE_COMPLETE]", path=".worktree/feature-x/progress.txt")`
   - Do NOT use `Bash("grep '[FEATURE_COMPLETE]' .worktree/feature-x/progress.txt")`
...
**CRITICAL**: If flow/tdd/confirm parameters were not specified on command line, check mega-plan.json for configuration from `/mega:plan`.
...
If prd_agent != "claude-code":
    If agents[prd_agent].type == "cli":
        If not is_command_available(agents[prd_agent].command):
            ...
            Else:
                echo "{prd_agent} not available, falling back to claude-code"
                prd_agent = "claude-code"
        Else:
...
If impl_agent != "claude-code":
    If agents[impl_agent].type == "cli":
        If not is_command_available(agents[impl_agent].command):
            ...
            Else:
                echo "{impl_agent} not available, falling back to claude-code"
                impl_agent = "claude-code"
        Else:
            # CRITICAL: Agent is available - YOU MUST USE IT
...
if all features complete:
    ...
    echo "BATCH COMPLETE WITH ERRORS"
    ...
    echo "AGENT FINISHED BUT FEATURE INCOMPLETE"
...
To resolve:
  ...
  3. git commit
  4. Re-run /mega:approve --auto-prd
...
Options:
  ...
  2. Re-run /mega:approve --auto-prd to retry
...
To fix:
  ...
  3. Re-run /mega:approve --auto-prd
...
/mega:approve --auto-prd
    ...
    |
    +--> ALL COMPLETE
        +--> Show final status, suggest /mega:complete
```
...
2. **Recommended**: Use `/mega:resume --auto-prd` to intelligently resume
   - Auto-detects actual state from files
   - Skips already-completed work
   - Compatible with old and new executions
3. Or re-run `/mega:approve --auto-prd` to continue from batch level
